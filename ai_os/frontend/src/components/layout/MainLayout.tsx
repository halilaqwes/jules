...
export function MainLayout() {
    ...
    // ⚡ Bolt: Memoize callbacks to prevent child re-renders on keystrokes
    const handleFileSelect = useCallback((path: string, content: string) => {
        setActiveFile(path);
        setCode(content);
    }, []);

    // ⚡ Bolt: Provide a stable reference so CodeEditor doesn't re-render
    // itself unnecessarily while typing
