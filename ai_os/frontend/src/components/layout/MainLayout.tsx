...
export function MainLayout() {
    ...
    // Memoize the onFileSelect handler to prevent recreating it on every CodeEditor typing re-render
    const handleFileSelect = useCallback((path: string, content: string) => {
        setActiveFile(path);
        setCode(content);
    }, []);

    // Memoize the onChange handler to prevent recreating it on every render, avoiding CodeEditor remount/thrashing
