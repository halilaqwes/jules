...
export function MainLayout() {
    ...
    // useCallback optimization: Prevents memoized FileExplorer from re-rendering on every keystroke in CodeEditor
    const handleFileSelect = useCallback((path: string, content: string) => {
        setActiveFile(path);
        setCode(content);
    }, []);

    // useCallback optimization: Prevents memoized CodeEditor from receiving a new function reference on every re-render
