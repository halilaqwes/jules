...
export function MainLayout() {
    ...
    // ⚡ Bolt Optimization: Memoize callback to prevent FileExplorer re-renders when MainLayout state changes
    // Impact: Avoids re-rendering FileExplorer when typing in the editor
    const handleFileSelect = useCallback((path: string, content: string) => {
        setActiveFile(path);
        setCode(content);
    }, []);

    // ⚡ Bolt Optimization: Memoize code change handler to prevent CodeEditor re-renders
    // Impact: Improves typing performance by preventing full layout re-renders on keystrokes
    ...
    return (
        <div className="flex h-screen w-screen bg-black overflow-hidden font-sans">
            {/* ⚡ Bolt Optimization: AgentSidebar is memoized to avoid re-renders */}
            <AgentSidebar />
            <FileExplorer
